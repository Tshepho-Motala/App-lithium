...
public class ProcessRewardService {
  ...
  private Long getRewardExpiryDate(ProcessRewardRequest request, ProviderConfig pc) {
    ...
    Granularity granularity = Optional.ofNullable(request.getRewardRevision().getValidForGranularity())
            .map(Granularity::fromGranularity).orElse(null);

    if ((request.getRewardRevision().getValidFor() != null) && (granularity != null)) {
