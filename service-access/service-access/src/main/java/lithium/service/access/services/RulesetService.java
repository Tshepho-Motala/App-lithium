...
public class RulesetService {
  ...
  public AccessRule editRule(RuleEdit ruleEdit, String authorGuid) throws Exception {
    if ("provider".equalsIgnoreCase(ruleEdit.getType())) {
      ExternalList providerRule = ruleEdit.getRuleset().getExternalList()
          .stream()
          .filter(r -> r.getId().equals(ruleEdit.getRuleId()))
          .findFirst()
          .orElseThrow(() -> new Status500InternalServerErrorException("Cant find external list id=[" + ruleEdit.getRuleId() + "]"));
      ...
      AccessControlList listRule = ruleEdit.getRuleset().getAccessControlList()
          .stream()
          .filter(r -> r.getId().equals(ruleEdit.getRuleId()))
          .findFirst()
          .orElseThrow(() -> new Status500InternalServerErrorException("Cant find access control list id=[" + ruleEdit.getRuleId() + "]"));
  ...
  public AccessRule deleteRule(AccessRule ruleset, Long ruleId, String type, String authorGuid) throws Exception {
    ...
    if ("provider".equalsIgnoreCase(type)) {
      ExternalList providerRule = ruleset.getExternalList()
          .stream()
          .filter(r -> r.getId().equals(ruleId)).findFirst()
          .orElseThrow(() -> new Status500InternalServerErrorException("Cant find external list id=[" + ruleId + "]"));
      ...
      AccessControlList listRule = ruleset.getAccessControlList()
          .stream()
          .filter(r -> r.getId().equals(ruleId))
          .findFirst()
          .orElseThrow(() -> new Status500InternalServerErrorException("Cant find access control list id=[" + ruleId + "]"));
  ...
  private List<ChangeLogFieldChange> saveRuleOutcomes(Map<String, String> outcomes,
    ...
    for (ExternalListRuleStatusOptionConfig ruleStatusOptionConfig : externalListRuleStatusOptionConfigList) {
      ...
      if (!outcomes.get(outcomeName).equalsIgnoreCase(outputName)) {
        log.debug(outcomeName + " changing from: " + outputName + " to: " + outcomes.get(outcomeName) + " [" + ruleStatusOptionConfig + "]");
  ...
  public AccessRule decreasePriority(AccessRule ruleset, Long ruleId, String type, String authorGuid) throws Exception {
    ...
    if ("provider".equalsIgnoreCase(type)) {
      ExternalList providerRule = ruleset.getExternalList().stream()
          .filter(r -> r.getId().equals(ruleId)).findFirst().orElse(null);
      changingPriority[0] = providerRule.getPriority();
      comment = providerRule.getListName();
    } else if ("list".equalsIgnoreCase(type)) {
