      ...
      <template  v-if="balanceMovementList">
        ...
        >
          ...
          <template v-slot:item.details.externalTranId="{ item }">
            ...
            <span @click="goToTransactionDetailUrl(item)" style="color: #1976d2;cursor: pointer;" v-if="item.details.externalTransactionDetailUrl"> {{ item.details.externalTranId }} </span>
            <span v-else>  {{ item.details.externalTranId }} </span>
...
export default class BalanceMovement extends Mixins(cashierMixins) {
  ...
  goToTransactionDetailUrl(item){
    if(item?.details?.externalTransactionDetailUrl && item.details.externalTransactionDetailUrl.includes('CashierTransactionID')){
      window.open(`#/dashboard/cashier/${item.account.domain.name}/view/${item.details.externalTranId}`, '_blank');
    } else {
      window.open(item.details.externalTransactionDetailUrl, '_blank');
    }
  }
