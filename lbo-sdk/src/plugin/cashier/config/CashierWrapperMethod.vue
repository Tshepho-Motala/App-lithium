  ...
  <div :data-test-id="`cashier-${type}-method`">
    <div class="mx-4">
      <div class="d-flex mb-3 align-center">
        ...
        <v-text-field
            data-test-id="txt-filtered-method"
  ...
  async getMethodAccounting(id: number) {
    ...
    try {
      ...
      if(list) {
        ...
        if ( res.length > 0 ) {
          let accountingInfo = {
            accountingDay: res[0]['day'],
            accountingWeek: res[0]['week'],
            accountingMonth: res[0]['month'],
            accountingLastMonth: res[0]['lastmonth']
          }
  ...
  async getMethodAccountingUser(id: number, username: string) {
    ...
    try {
      ...
      if (list) {
        ...
        if ( res.length > 0 ) {
          let accountingInfo = {
            accountingUserDay: res[0]['day'],
            accountingUserWeek: res[0]['week'],
            accountingUserMonth: res[0]['month'],
            accountingUserLastMonth: res[0]['lastmonth']
          }
          return accountingInfo
        }
      }
