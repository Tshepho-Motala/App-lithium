  ...
  <v-expansion-panel data-test-id="cashier-method" :class="isDisabled ? 'method-disabled-shadow' : ''" class="mb-1">
    <v-expansion-panel-header ripple="true">
      <div class="d-flex justify-space-between">
        ...
        <div class="d-flex align-center">
            <table data-test-id="tbl-method-accounting" class="table table-sm description-sm mb-0" v-if="method.accountingDay || method.accountingWeek || method.accountingMonth || method.accountingLastMonth">
              <tbody>
              <tr>
                <td data-test-id="tbi-accounting-general" class="domain d-flex flex-column">
                ...
                <td data-test-id="tbi-accounting-user" class="user d-flex  flex-column" v-if="viewAs === 'user'">
                  <div class="bold-text">{{$translate(`UI_NETWORK_ADMIN.CASHIER_CONFIG.DOMAIN_${getMethodType()}_TOTALS`)}}</div>
 ...
 async modifyMethod() {
  ...
  async getProcessorAccounting(id: number) {
    try {
      ...
      if(result) {
        const res = result.plain()

        if (res.length > 0) {
          this.tempMethodProcessors.forEach((item) => {
            if (item.id === id) {
              item.accountingDay = res[0]['day'];
              item.accountingWeek = res[0]['week'];
              item.accountingMonth = res[0]['month'];
              item.accountingLastMonth = res[0]['lastmonth'];
            }
          })
        }
      }
  ...
  async getProcessorAccountingUser(id: number, username: string) {
    try {
      ...
      if(list) {
        ...
        const res = list.plain()

        if(res.length > 0) {
          this.tempMethodProcessors.forEach((item) => {
            if(item.id === id) {
              item.accountingUserDay = res[0]['day']
              item.accountingUserWeek = res[0]['week']
              item.accountingUserMonth = res[0]['month']
              item.accountingUserLastMonth = res[0]['lastmonth']
            }
          })
        }
      }
