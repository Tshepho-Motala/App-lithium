      ...
      <v-tabs-items v-model="mainTab">
        ...
        >
          <v-card flat>
            ...
            <!--            PROFILES           -->
            <div v-if="tab.name === 'Profiles'">
              <cashier-profile :domain="activeDomain.name" />
            </div>
            <!--            INNER ITEMS        -->
...
import CashierProfile from "@/plugin/cashier/config/CashierProfile.vue";
...
export default class CashierConfigPage extends Mixins(TranslationMixin) {
  ...
  mainTabItems: MainTab[] = [
    ...
    {
      name: 'Profiles',
      displayName: this.$translate('UI_NETWORK_ADMIN.CASHIER_CONFIG.PROFILES_'),
      id: 2
    }
  ]
