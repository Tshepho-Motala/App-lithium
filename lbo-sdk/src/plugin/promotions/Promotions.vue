<template>
  <div style="width: 100%">
    <v-alert type="error"
             v-if="domain && !domain.timezone">
      Error: Domain does not have a timezone.
    </v-alert>
...
export default class Promotions extends Vue {
  ...
  async showEventsOnCalendar() {
    if (!this.calStartDate || !this.calEndDate || this.domain === null || !this.domain.timezone) {
      return
    }
  ...
  onCreatePromotionClick(date: string | undefined) {
    ...
    if (this.domain !== null) {
      ...
      if (this.domain.timezone && date) {
        const [year, month, day] = date.split('-').map(x => parseInt(x))
        const utcDateS = Date.UTC(year, month - 1, day)
        const utcDate = new Date(utcDateS)
        this.draftPromotion.schedule.preselectedStartUtc = utcDate
      }

      this.draftPromotion.domain = this.domain
      this.draftPromotion.schedule.timezone = this.domain.timezone
    }
  ...
  // Save the promotion - get the revision id
  // Set the list of exlusive players by revision ID
  async onPromotionSave() {
    ...
    if (this.draftPromotion !== null) {
      try {
        ...
        if (draft !== null) {
          if (draft.edit !== null && draft.edit.id !== undefined && this.draftPromotion.pendingExclusivePlayers && this.draftPromotion.pendingExclusivePlayers.length > 0) {
            await this.apiClients.servicePromo.addExclusivePlayers(draft.id.toString(), this.draftPromotion.pendingExclusivePlayers)
          }

          if (this.publishAfterSaving) { await this.apiClients.servicePromo.publishPromotionDraft(draft.id.toString()) }
        }
      } catch (e) {
        console.error(e)
        this.loading = false
