<template>
  <DomainSelectorPage v-model="promotion.domain"
                      ref="domainSelector"
                      :title="titleDisplay"
    ...
    <template>
      <div style="max-height: 800px">
        <v-tabs-items v-model="tabModel"
          ...
          <v-tab-item>
            <v-row>
              <v-col cols="12">
                <v-select outlined
                          disabled
                          label="Select Promotion Dependency"></v-select>
              </v-col>
              <v-col cols="8">
                <BasicDetails :promotion="promotion" />
              </v-col>
              <v-col cols="4">
                <StepViewReward v-if="hasReward"
                                :reference="promotion.reward"
                                :domain="promotion.domain"
                                :color="promotion.theme.color"
                                @delete="onRewardDelete" />
                <StepAddReward v-if="!hasReward"
                               :domain="promotion.domain"
                               :color="promotion.theme.color"
                               @save="onRewardSaved" />
              </v-col>
                ...
                <div>
                  <v-alert :color="color"
                    ...
                    <span class="text-body-2">
                      Create challenges for the player to complete via <strong>Challenge Paths</strong>. A player can
                      complete any
                      <strong>Path</strong> to activate the <strong>Promotion Reward</strong>. All
                      <strong>Challenges</strong> in a
                      <strong>Path</strong> are required to be completed before the <strong>Path</strong> is
                      complete.</span>
                  </v-alert>
                </div>
              </v-col>
            ...
            <ChallengeGroupEdit v-model="promotion"
                                class="pt-3" />
          </v-tab-item>
          <v-tab-item>
          ...
          <v-tab-item>
            <ExclusiveConfig v-model="promotion" />
          </v-tab-item>
          <v-tab-item>
            <TagBlacklistWhitelist v-model="promotion"
                                   :domain="promotion.domain" />
          </v-tab-item>
        </v-tabs-items>
      </div>
...
export default class PromotionEdit extends Vue {
  ...
  get hasSchedule(): boolean {
    ...
    return !!this.promotion.schedule.dateTimeStartUtc && !!this.promotion.schedule.rruleString && hasRedemption
  }

  get hasExclusiveRedemption() {
    if (!this.promotion.exclusive) {
      return true
    }
    if (Number.isNaN(this.promotion.redeemOverEntry)) {
      return false
    }
    if (this.promotion.redeemOverEntry && parseInt(this.promotion.redeemOverEntry) > 0) {
      return true
    }
    return false
  }
  ...
  get canSave(): boolean {
    return this.hasDomain && this.hasBasicDetails && this.hasReward && this.hasSchedule && this.hasExclusiveRedemption
  }
