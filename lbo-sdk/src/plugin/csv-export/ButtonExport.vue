      ...
      <template v-slot:activator="{ on, attrs }">
        <div v-bind="attrs" v-on="on">
          ...
          <template v-if="canBeProcessed">
            <div>
              <v-btn :disabled="disableButton" @click="onGenerateClick"
...
export default class ButtonExport extends Vue {
  ...
  mounted() {
    if(this.hasProps && this.params !== undefined){
      this.config = this.params
    } else {
      this.config = this.csv.getConfig()
    }
    const hasPermission = this.hasPermission()
    this.disableButton = !hasPermission
  ...
  get canBeProcessed(): boolean {
    return !this.failed && !isEmpty(this.progress) && this.progress.status != ExportStatus.COMPLETE && this.progress.status != ExportStatus.BUSY && this.progress.status != ExportStatus.CREATED
  }
