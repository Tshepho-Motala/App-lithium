<template>
  <v-card flat
          id="DomainSelectorPage">
    <v-card-title v-if="!invisible">
      <v-toolbar :short="!extended"
        ...
        <div>
          ...
          <v-tooltip v-if="hasDomain && !readonly"
                     top
                     v-model="showDomainTooltip">
            <template v-slot:activator="{ on, attrs }">
              <v-btn small
                     icon
                     :outlined="showDomainTooltip"
                     class="mt-n1 ml-2"
                     color="white"
                     v-bind="attrs"
                     v-on="on"
    ...
    <template v-if="showDomainSelector">
      <v-card-text>
        {{ domain }}
        <DomainSelectorSimple v-model="domain"
                              @change="onChange"
                              :roles="roles"
                              :combineApiData="combineApiData" />
        <div class="px-4">
