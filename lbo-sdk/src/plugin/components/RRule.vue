                  ...
                  <div class="d-flex flex-row justify-start"
                    ...
                    <div>
                      ...
                      <div class="text-caption">
                        <TimeViewer :utcDate="item.startUtc"
                                    :timezone="timezone" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div class="text-caption">
                        <TimeViewer :utcDate="item.endUtc"
                                    :timezone="timezone" />
                      </div>
                    </div>
                  </div>
...
import { addYears, subSeconds, addDays, format } from 'date-fns'
 ...
 * 
 * The calendar and schedule breakdown needs to display based on DOMAIN timezone.
 * The time selected is selected by the user based on the DOMAIN timezone.
 * 
 * This means that DISPLAY times are in DOMAIN timezone, and
 *                 SAVE    times are in UTC    timezone
 */
@Component({
  components: {
    TimeSelector,
  ...
  */
  ...
  created() {
    ...
    if (this.schedule !== null) {
      if (this.schedule.dateTimeStartUtc) {
        this.dateStart = formatInTimeZone(this.schedule.dateTimeStartUtc, this.timezone, 'yyyy-MM-dd')
        this.timeStart = formatInTimeZone(this.schedule.dateTimeStartUtc, this.timezone, 'HH:mm')
        this.calendarValue = this.dateStart
      }

      if(this.schedule.preselectedStartUtc) {
        this.schedule.dateTimeStartUtc = this.schedule.preselectedStartUtc
        this.dateStart = formatInTimeZone(this.schedule.dateTimeStartUtc, 'UTC', 'yyyy-MM-dd')
        this.timeStart = formatInTimeZone(this.schedule.dateTimeStartUtc, 'UTC', 'HH:mm')
        this.calendarValue = this.dateStart
      }

      if (this.schedule.dateTimeUntilUtc) {
        this.dateEnd = this.schedule.dateTimeUntilUtc.toLocaleDateString('en-CA')
      }
  ...
  private visualiseIndividualEvents() {
    ...
    // We select time in the domain timezone,
    // but we need to convert it to UTC
    // as the backend always uses UTC
    const time = this.timeStart ? this.timeStart + ':00' : "00:00:00"
    const dtstart = zonedTimeToUtc(`${this.dateStart} ${time}`, this.timezone, {
    ...
    if (this.calendarStart && this.calendarEnd) {
      ...
      for (const start of eventStartDates) {
        const modifier = this.eventLengthInDays
        const end = addDays(start, modifier)

        // Convert the UTC time to selected timezone
        // to display on calendar
        const startLocal = utcToZonedTime(start, this.timezone)
        const endLocal = utcToZonedTime(end, this.timezone)

        // Looks like dates for calendar need to be in browser TZ
        this.events.push({
          start: startLocal,
          startUtc: start,
