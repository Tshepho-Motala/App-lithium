...
export default class AxiosApiClient extends ApiClient<AxiosInstance> {
  ...
  async postJsonWithParameters<T>(data: any, parameters: { [key: string]: string }, ...urlParts: string[]): Promise<null | T> {
    const baseURL = this.getBaseUrlByEnvironment()
    const url = this.path(baseURL, ...urlParts)
    const parameterList: string[] = []
    for (const key of Object.keys(parameters)) {
    ...
    const response = await this.provider.post(urlWithParameters, JSON.stringify(data), { headers })
   ...
   * @returns null if success, anything else if error
   */
  async putWithParameters<T>(parameters: { [key: string]: string }, ...urlParts: string[]): Promise<null | T> {
    const baseURL = this.getBaseUrlByEnvironment()
    const url = this.path(baseURL, ...urlParts)
    const parameterList: string[] = []
    for (const key of Object.keys(parameters)) {
    ...
    const response = await this.provider.put(urlWithParameters, {}, { headers })
