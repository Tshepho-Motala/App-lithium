import {
  ...
  PromotionHistoricExclusivePlayer
...
import { PageContract, TableContract } from '../generic/TableContract'
...
export default class ServicePromoClient extends AxiosApiClient {
  ...
  addExclusivePlayers(promotionId: string, playerGuids: string[]): Promise<PromotionExclusivePlayer | null> {
    return this.postJson(
      {
        promotionId,
        players: playerGuids
  ...
  getPromotionExclusiveListTable(
    promotionId: string,
    page: string,
    size: string,
    status: string | null = null
  ): Promise<PageContract<PromotionExclusivePlayer> | null> {
    const requestData = {
  ...
  getPromotionExclusiveListProgress(promoExclusiveUploadId: string): Promise<PromoExclusiveStats | null> {
    return this.get('exclusive-promotion', 'v1', promoExclusiveUploadId, 'progress')
  }

  getPromotionValidExclusiveListTable(
    promotionId: string,
    page: string,
    size: string
  ): Promise<PageContract<PromotionHistoricExclusivePlayer> | null> {
    const requestData = {
      promotionId: promotionId,
      page,
      size
