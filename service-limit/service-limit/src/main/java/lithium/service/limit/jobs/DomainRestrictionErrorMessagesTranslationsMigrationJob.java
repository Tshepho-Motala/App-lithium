...
public class DomainRestrictionErrorMessagesTranslationsMigrationJob {
    ...
    public void checkErrorMessagesTranslations(int pageSize, Long delay) throws InterruptedException {
        log.debug("Error Message translations Migration job requested with: pageSize={} delay={} isMigrationJobStarted={}", pageSize, delay, JobState.isStarted);
        if (!JobState.isStarted) {
            ...
            log.debug("Error Message translations check started for:" + total + " messages");
            ...
            JobState.finish();
            log.debug(":: Migration of Error Message translations  is finished." + updatedMessagesCount.get() + " Messages precessed");
        } else {
            log.debug("Migration is running:" + updatedMessagesCount.get() + " of " + total + " completed");
        }
    }
    ...
    public void terminate() {
        JobState.terminate();
        log.debug("Error Message translations MigrationJob is terminated with " + updatedMessagesCount.get() + " of " + total + " messages processed");
        updatedMessagesCount = new AtomicLong(0);
        total = 0L;
    }
