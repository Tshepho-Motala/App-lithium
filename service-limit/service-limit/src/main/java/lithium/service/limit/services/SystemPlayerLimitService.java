...
public class SystemPlayerLimitService {
    ...
    public List<PlayerLimit> setUserLimit(User user) {
        ...
        lithium.service.limit.data.entities.DomainLimit domainLimitYear = domainLimitRepo.findByDomainNameAndGranularityAndType(domain, 1, 2);
        ...
        if (domainLimitYear != null) {
            try {
                playerLimits.add(storeDomainLimit(domainLimitYear, guid, id));
            } catch (Exception n) {
                log.error("Problem setting system limit service on monthly limit", n);
            }
        }
    ...
    public lithium.service.limit.client.objects.User getLossLimitVisibility(String playerGuid) {
        return playerLimitService.getLossLimitVisibility(playerGuid);
    }
