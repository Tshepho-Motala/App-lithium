...
public class PlayerLimitService {
    ...
    public void checkLimits(final String playerGuid, final String domainName, final String currency,
            ...
            Status478TimeSlotLimitException, Status475AnnualLossLimitReachedException {
    ...
    private void verifyLossLimits(String playerGuid, String domainName, String currency, Locale locale,
        ...
        // Loss limit (year)
        if (playerLossLimitYear != null && (netLossYear + currentLoss) > playerLossLimitYear.getAmount()) {
            setLossLimitVisibility(playerGuid);
            sendLossLimitVisibilityNotifications(playerGuid,LossLimitVisibilityMessageType.LOSS_LIMIT_REACHED);
            throw new Status475AnnualLossLimitReachedException(
                    messageSource.getMessage("SERVICE-LIMIT.ANNUAL_LOSS_LIMIT_REACHED", new Object[]{
                            toCurrency(playerLossLimitYear.getAmount(), currency)}, locale)
            );
        } else if (domainLossLimitYear != null && playerLossLimitYear == null
                && (netLossYear + currentLoss) > domainLossLimitYear.getAmount()) {
            setLossLimitVisibility(playerGuid);
            sendLossLimitVisibilityNotifications(playerGuid,LossLimitVisibilityMessageType.LOSS_LIMIT_REACHED);
            throw new Status475AnnualLossLimitReachedException(
                    messageSource.getMessage("SERVICE-LIMIT.ANNUAL_LOSS_LIMIT_REACHED", new Object[]{
                            toCurrency(domainLossLimitYear.getAmount(), currency)}, locale)
            );
        }
    }
    ...
    public PlayerLimitV2Dto findPlayerLimitV2WithNetLoss(String domainName, String playerGuid, int playerAge, int granularity, int type) {
        ...
        if (playerLimit == null) {
            DomainAgeLimit domainAgeLimit = ageLimitService.findDomainAgeLimit(domainName, granularity, type, playerAge);
            if (domainAgeLimit != null) {
                limitAmountCents = domainAgeLimit.getAmount();
            }
            if (limitAmountCents != null) {
                DomainLimit domainLimit = domainLimitRepo.findByDomainNameAndGranularityAndType(domainName, granularity, type);
                if (domainLimit != null) {
...
//			oldPlayerLimit.setAmount(amount);
            playerLimit.setAmount(amount);
            playerLimit = playerLimitRepo.save(playerLimit);

            addHistory(playerLimit, ModifyType.UPDATED, tokenUtil.guid());
	        ...
	        formatCurrencyFields(clfc, domain.getDefaultLocale(), domain.getCurrencySymbol(),domain.getCurrency());
            changeLogService.registerChangesForNotesWithFullNameAndDomain("user.limit", "edit", playerId, tokenUtil.guid(), tokenUtil,
                    null, null, clfc, Category.RESPONSIBLE_GAMING, SubCategory.LOSS_LIMITS, 40, domainName);
            pubSubUserAccountChangeProxy.listenAccountChanges(playerGuid);
            return playerLimit;
        }
        ...
        if (playerLimit == null && amount > 0) {
            ...
            addHistory(playerLimit, ModifyType.CREATED, tokenUtil.guid());

            ChangeLogLimit changeLogLimit = convertToCLLimit(playerLimit);
            String[] fields = {"playerGuid", "domainName", "granularity", "amount", "type"};
            List<ChangeLogFieldChange> clfc = changeLogService.copy(changeLogLimit, new ChangeLogLimit(), fields);
	        formatCurrencyFields(clfc, domain.getDefaultLocale(), domain.getCurrencySymbol(),domain.getCurrency());
            changeLogService.registerChangesForNotesWithFullNameAndDomain("user.limit", "create", playerId, tokenUtil.guid(), tokenUtil,
                    null, null, clfc, Category.RESPONSIBLE_GAMING, SubCategory.LOSS_LIMITS, 40, domainName);
            pubSubUserAccountChangeProxy.listenAccountChanges(playerGuid);
            return playerLimit;
        }
    ...
    public lithium.service.limit.client.objects.User getLossLimitVisibility(String playerGuid) {
        lithium.service.limit.data.entities.User  player = userRepository.findOrCreateByGuid(playerGuid, () -> new User());
