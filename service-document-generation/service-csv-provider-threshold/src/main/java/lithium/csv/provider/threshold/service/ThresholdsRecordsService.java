...
public class ThresholdsRecordsService implements CsvProviderAdapter<ThresholdsFilterRequestParams> {
    ...
    private ThresholdLossLimitHistoryCsv buildLossLimitThresholdHistoryCsv(PlayerThresholdHistoryDto t) {
        ...
        historyCsv.setLossLimit(getNonNullValue(getLossLimit(t)));
        historyCsv.setLossLimitUsed(getNonNullValue(getLossLimitUsed(t)));
        historyCsv.setAccountCreationDate(t.getAccountCreationDate().toString());
        historyCsv.setThreshold(getNonNullValue(t.getThresholdRevision().getPercentage().toPlainString())+"%");
    ...
    private ThresholdDepositHistoryCsv buildDepositThresholdHistoryCsv(PlayerThresholdHistoryDto t) {
        ...
        historyCsv.setAccountCreationDate(t.getAccountCreationDate().toString());
        historyCsv.setThreshold(getNonNullValue(t.getAmount().toPlainString()));
    ...
    private String getLossLimit(PlayerThresholdHistoryDto t) {
        return switch (Granularity.fromType(t.getGranularity())) {
            case GRANULARITY_DAY -> t.getDailyLimit()!=null?t.getDailyLimit().toPlainString():"";
            case GRANULARITY_WEEK -> t.getWeeklyLimit()!=null?t.getWeeklyLimit().toPlainString():"";
            case GRANULARITY_MONTH -> t.getMonthlyLimit()!=null?t.getMonthlyLimit().toPlainString():"";
            case GRANULARITY_YEAR -> t.getAnnualLossLimit()!=null?t.getAnnualLossLimit().toPlainString():"";
            default -> "";
        };
    }
    private String getLossLimitUsed(PlayerThresholdHistoryDto t) {
        return switch (Granularity.fromType(t.getGranularity())) {
            case GRANULARITY_DAY -> t.getDailyLimitUsed()!=null?t.getDailyLimitUsed().toPlainString():"";
            case GRANULARITY_WEEK -> t.getWeeklyLimitUsed()!=null?t.getWeeklyLimitUsed().toPlainString():"";
            case GRANULARITY_MONTH ->t.getMonthlyLimitUsed()!=null?t.getMonthlyLimitUsed().toPlainString():"";
            case GRANULARITY_YEAR -> t.getAnnualLossLimitUsed()!=null?t.getAnnualLossLimitUsed().toPlainString():"";
            default -> "";
        };
    }
