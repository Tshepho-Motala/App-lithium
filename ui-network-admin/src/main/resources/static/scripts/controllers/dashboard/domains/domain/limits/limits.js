...
angular.module('lithium')
	...
	function(domain, notify, $uibModal, $scope, domainLimitsRest, errors, bsLoadingOverlayService) {
		...
		$scope.refresh = function() {
			...
			domainLimitsRest.findDomainLimit(domain.name, domainLimitsRest.GRANULARITY_YEAR, domainLimitsRest.LIMIT_TYPE_LOSS).then(function(response) {
			...
			var modalInstance = $uibModal.open({
				...
				resolve: {
					...
					annualLimit: function () {
						if (granularity
							=== domainLimitsRest.GRANULARITY_YEAR) {
							if (type === domainLimitsRest.LIMIT_TYPE_WIN) {
								return undefined;
							} else if (type
								=== domainLimitsRest.LIMIT_TYPE_LOSS) {
			...
			modalInstance.result.then(function (result) {
				if (result) {
					if (granularity === domainLimitsRest.GRANULARITY_DAY) {
						...
						if (type === domainLimitsRest.LIMIT_TYPE_WIN) {
							$scope.annualWinLimit = result.amount / 100;
						} else if (type === domainLimitsRest.LIMIT_TYPE_LOSS) {
							$scope.annualLossLimit = result.amount / 100;
						}
					}
