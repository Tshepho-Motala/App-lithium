...
angular.module('lithium').controller('AutoRestrictionRulesetRuleModal', ["domainName", "ruleset", "rule", "filterFields", "filterOperators", "errors", "$scope", "notify", "$uibModalInstance", "bsLoadingOverlayService", "AutoRestrictionRulesetRest", "VerificationStatusRest", "RestrictionsRest", "BiometricsStatusRest",
    function (domainName, ruleset, rule, filterFields, filterOperators, errors, $scope, notify, $uibModalInstance, bsLoadingOverlayService, rest, verificationStatusRest, restrictionsRest, biometricsStatusRest) {
        var controller = this;
                ...
                controller.fields.push({
                    ...
                    controller: ['$scope', function($scope) {
                        ...
                        if (controller.model.field === 1) { // VERIFICATION_STATUS
                            ...
                            controller.model.operator = 1;
                        } else if (controller.model.field === 11) { // BIOMETRIC_STATUS
                            for (var i = 0; i < operators.length; i++) {
                                if (operators[i].id === 6) { // IN
                                    filteredOperators.push(operators[i]);
                                    break;
        ...
        controller.onSubmit = function() {
            ...
            if (model.field === 1 || model.field === 11) { //VERIFICATION_STATUS OR BIOMETRIC_STATUS
                var value = '';
