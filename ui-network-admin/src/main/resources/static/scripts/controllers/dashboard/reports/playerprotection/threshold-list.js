...
angular.module('lithium').controller('ThresholdListReport', ['errors', '$translate', '$dt', 'DTOptionsBuilder', '$filter', '$scope', '$state' ,'$rootScope', '$userService', 'rest-domain'
    ,'bsLoadingOverlayService','userThresholdHistoryRest','$http','$stateParams','DocumentGenerationRest','UserRest',
    ...
    ,userThresholdHistoryRest, $http, $stateParams, documentRest,userRest) {
        var controller = this;
        ...
        .column($dt.columnperiod('threshold.granularity').withTitle($translate("Threshold Hit")))
        ...
        .column($dt.column('id').withTitle($translate("Loss Limit")).renderWith(function (data, type, row) {
            switch (row.threshold.granularity) {
                case '1':
        ...
        .column($dt.column('id').withTitle($translate("Threshold")).renderWith(function (data, type, row, meta) {
            if (row.thresholdRevision.percentage !== null) return row.thresholdRevision.percentage + "%";
            return "";
        }))
        ...
        controller.thresholdHistoryTable = dtBuilder.build();
        ...
        $scope.reference = () => {
            if (controller.model?.selectedDomain) {
                const reference = localStorage.getItem(`export_reference_thresholds_${controller.model.selectedDomain}`)
                if (angular.isUndefinedOrNull(reference)) {
                    return null;
                }
                return reference.replace('export_reference_thresholds_', '')
            }
            const reference = localStorage.getItem(`export_reference_thresholds_list`)

            if (angular.isUndefinedOrNull(reference)) {
                return null;
            }

            return reference.replace('export_reference_thresholds_list', '')
        }
        ...
        $scope.apiParams = () => {
            ...
            params.typeName = "TYPE_LOSS_LIMIT";
        ...
        $rootScope.provide.csvGeneratorProvider.generate = async (config) => {
            ...
            if (controller.model?.selectedDomain) {
                localStorage.setItem(`export_reference_thresholds_${controller.model.selectedDomain}`, response.reference);
            } else {
                localStorage.setItem(`export_reference_thresholds_list`, response.reference);
            }
            return response
        }
