...
// url:'/:id',
		...
		resolve: {
			...
			limits: ["user", "domainInfo", "userLimitsRest", "errors" ,'$q', function(user, domain, userLimitsRest, errors, $q) {
				...
				// annual loss limit
				var allPromise = userLimitsRest.findPlayerLimit(user.guid, user.domain.name, userLimitsRest.GRANULARITY_YEAR, userLimitsRest.LIMIT_TYPE_LOSS).then(function(response) {
					return (response.amount != undefined)? response.amount / 100: null;
				}).catch(function() {
					errors.catch('', false)
				});
				...
				//annual Net Loss To House
				var anlPromise = userLimitsRest.findNetLossToHouse(user.domain.name, user.guid, domain.currency, userLimitsRest.GRANULARITY_YEAR).then(function(response) {
					return (response != undefined)? response / 100: null;
				}).catch(function() {
					errors.catch('', false);
				});

				return $q.all([dllPromise, wllPromise, mllPromise, allPromise, dnlPromise, wnlPromise, mnlPromise, anlPromise]).then(function(results){
					return {
						dailyLossLimit: results[0],
						weeklyLossLimit: results[1],
						monthlyLossLimit: results[2],
						annualLossLimit: results[3],
						dailyNetLossToHouse: results[4],
						weeklyNetLossToHouse: results[5],
						monthlyNetLossToHouse: results[6],
						annualNetLossToHouse: results[7],
						domain: domain
					};
				});
			}],
				...
				return userLimitsRest.depositLimits(user.guid).then(function(response) {
					...
					angular.forEach(response.plain(), function(v, k) {
						if (v.granularity === userLimitsRest.GRANULARITY_DAY) {
							...
							data.monthlyLimit = v.amount / 100;
							data.monthlyLimitUsed = v.amountUsed / 100;
						} else if (v.granularity === userLimitsRest.GRANULARITY_YEAR) {
							data.annualLimit = v.amount / 100;
							data.annualLimitUsed = v.amountUsed / 100;
						}
					});
					return data;
				}).catch(
	...
	$stateProvider.state('dashboard.players.player.kyc_entry', {
		template: '<changelogentry backhref="dashboard.players.player.kyc" entry="$resolve.entry"/>',
		url:'/:entryid',
		resolve: {
			entry: ["ChangelogsRest", "$stateParams", "$rootScope", function(rest, $stateParams, $rootScope) {
				return rest.changeLogWithFieldChanges($stateParams.entryid);
		...
		ncyBreadcrumb: {
			parent: 'dashboard.players.player.kyc',
			label: 'player changelog entry'
		}
	});
...
// parent: 'dashboard.bonuses',
	...
	$stateProvider.state('dashboard.casino.bet-history', {
		templateUrl:'scripts/controllers/dashboard/casino/bet-history/domain-select.html',
		data: {
			pageTitle: "UI_NETWORK_ADMIN.BET_HISTORY.HEADER.TITLE",
		},
		controller:'BetHistoryDomainSelectController as controller',
		url:'/bet-history',
		resolve: {
			loadMyFiles:function($ocLazyLoad) {
				return $ocLazyLoad.load({
					name:'lithium',
					files: [
						'scripts/controllers/dashboard/casino/bet-history/domain-select.js'
					]
				})
			}
		},
	...
	$stateProvider.state('dashboard.casino.bet-history.list',{
		templateUrl: 'scripts/controllers/dashboard/casino/bet-history/list/list.html',
		url:'/{domain}',
		controller:'CasinoHistoryController as controller',
		resolve: {
			user: Object,
			...
			loadMyFiles:function($ocLazyLoad) {
				return $ocLazyLoad.load({
					name:'lithium',
					files:['scripts/controllers/dashboard/casino/bet-history/list/list.js']
				})
			}
		},
	});
