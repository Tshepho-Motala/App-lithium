...
['$uibModalInstance', 'user', 'dailyLossLimit', 'weeklyLossLimit', 'monthlyLossLimit','annualLossLimit', 'granularity', '$scope', 'notify', 'errors', 'bsLoadingOverlayService', 'userLimitsRest', 'domainLimitsRest', '$translate',
function ($uibModalInstance, user, dailyLossLimit, weeklyLossLimit, monthlyLossLimit,annualLossLimit, granularity, $scope, notify, errors, bsLoadingOverlayService, userLimitsRest, domainLimitsRest, $translate) {
	var controller = this;
	...
	controller.init = function() {
		if (granularity === userLimitsRest.GRANULARITY_DAY) {
			...
			controller.type = 'Annual';
			controller.model.annualLossLimit = annualLossLimit;
			controller.fields.push({
				className: 'col-xs-12',
				key: 'annualLossLimit',
				type: 'ui-money-mask',
				...
				expressionProperties: {
					'templateOptions.label': '"UI_NETWORK_ADMIN.LIMITS.PLAYER.ANNUALLOSSLIMIT.LABEL" | translate',
					'templateOptions.placeholder': '"UI_NETWORK_ADMIN.LIMITS.PLAYER.ANNUALLOSSLIMIT.LABEL" | translate',
					'templateOptions.description': '"UI_NETWORK_ADMIN.LIMITS.PLAYER.ANNUALLOSSLIMIT.DESCRIPTION" | translate'
				}
			});
		}
	}
	...
	controller.submit = function() {
		...
		if (granularity === userLimitsRest.GRANULARITY_DAY) {
			...
			limit = controller.model.annualLossLimit;
		}
		limit = limit * 100;
