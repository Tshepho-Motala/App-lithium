...
angular.module('lithium')
    .controller('DocumentVerificationModal', ['user', '$uibModalInstance', '$scope', '$state', '$translate', 'notify', 'DocumentVerifyRest',
        function (user, $uibModalInstance, $scope, $state, $translate, notify,  documentVerifyRest) {
            ...
            controller.requireBiometricCheck = function() {
                var requireBiometricIdentity = ['required_photo',  'required_video'].includes(user.biometricsStatus);
                if (requireBiometricIdentity) {
                    controller.model.report = "Document verification unavailable for users which require biometric validation"
                }
                return requireBiometricIdentity;
            }
