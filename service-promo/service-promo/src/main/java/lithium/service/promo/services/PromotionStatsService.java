...
public class PromotionStatsService {
	...
	@Autowired
	PromoExclusivePlayerRepository promoExclusivePlayerRepository;
			...
			if (externalUser.getUserCategories() != null) {
				...
				for (lithium.service.promo.data.entities.Promotion promotion: promotions) {
					...
					if (Boolean.TRUE.equals(current.getExclusive())) {
						...
						Optional<PromoExclusivePlayer> exclusivePlayer = promoExclusivePlayerRepository.findOneByPlayerGuidAndPromotionId(promoContext.playerGuid(), promotion.getId());

						if (exclusivePlayer.isPresent()) {
							log.debug("Guid: {} found in exclusive list, adding promo.", promoContext.playerGuid());
							promoContext.setLastAddedToExclusiveList(exclusivePlayer.get().getLastAdded());
