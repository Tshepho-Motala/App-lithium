...
public class PromotionService {
	...
	@Autowired
	private ExclusivePromoService exclusivePromoService;
			...
			throws Status410InvalidPromotionCreateException, Status411InvalidUserCreateException, ParseException
	{
		...
		LocalDateTime startDate = parseDateTimeStringToLocalDateTime(editRevisionPost.getStartDate());
		LocalDateTime endDate = parseDateTimeStringToLocalDateTime(editRevisionPost.getEndDate());
		...
		PromotionRevision editRevision = PromotionRevision.builder()
				...
				.redeemablePerEntry(editRevisionPost.getRedeemablePerEntry())
	...
	private PromotionRevisionBO convertPromotionRevisionBO(PromotionRevision promotionRevision) {
		...
		return PromotionRevisionBO.builder()
				...
				.redeemablePerEntry(promotionRevision.getRedeemablePerEntry())
				.build();
	}
	...
	public ExclusiveUploadProgress uploadExclusivePlayers(Set<String> exclusivePlayers, Long promotionId, ExclusiveUploadType type)
	{
		lithium.service.promo.data.entities.Promotion promotion = findPromotion(promotionId).orElseThrow(Status410InvalidPromotionCreateException::new);

		var users = exclusivePlayers.stream().map(u -> lithium.service.promo.client.objects.User.builder()
						.guid(u).build())
				.collect(Collectors.toSet());

		return exclusivePromoService.upload(ExclusiveUploadData.builder()
...
//	}
	...
	public LocalDateTime parseDateTimeStringToLocalDateTime(String datetimeString) {
		...
		return LocalDateTime.from(dateTimeFormatter.parse(datetimeString)); //This is the server local datetime which is always
	}
}
