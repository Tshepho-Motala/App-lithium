...
public class ServicePromotionsModuleInfo extends ModuleInfoAdapter {
	...
	public void configureHttpSecurity(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/backoffice/exclusive-promotion/v1/**").access("@lithiumSecurity.hasRole(authentication, 'PROMOTIONS_EDIT')");
...
//		http.authorizeRequests().antMatchers("/admin/promotion/{id}/challenge/{challengeId}/getIcon").access("@lithiumSecurity.hasDomainRole(authentication, 'MISSIONS_VIEW', 'MISSIONS_EDIT')");
		...
		http.authorizeRequests().antMatchers("/external/remove-players").permitAll();
		http.authorizeRequests().antMatchers("/external/{promoExclusiveUploadId}/progress").permitAll();
