...
public interface UserPromotionRepository extends PagingAndSortingRepository<UserPromotion, Long>, JpaSpecificationExecutor<UserPromotion> {
	...
	@Query("select o from #{#entityName} o LEFT OUTER JOIN o.user u LEFT OUTER JOIN o.promotionRevision mr LEFT OUTER JOIN o.period p where u.guid = :guid and mr.id = :promotionRevisionId and p.id = :periodId and o.started >= :entryDate")
	@Lock( LockModeType.PESSIMISTIC_FORCE_INCREMENT)
	List<UserPromotion> findByUserGuidAndPromotionRevisionAndPeriodAndStartedAfterEntryDateForUpdate(@Param("guid") String guid, @Param("promotionRevisionId") Long promotionRevisionId, @Param("periodId") Long periodId, LocalDateTime entryDate);
