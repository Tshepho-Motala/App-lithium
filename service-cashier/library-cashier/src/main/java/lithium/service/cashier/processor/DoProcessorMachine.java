...
public class DoProcessorMachine {
	...
	public DoProcessorResponse run(DoProcessorRequest request) {
		log.debug("DoProcessorMachine run request " + request);
		...
		try {
			...
			log.debug("DoProcessorMachine "+request.getTransactionType().description().toLowerCase()+" run response " + request.toString() + " " + response.toString());
			return response;
		} catch (Throwable t) {
