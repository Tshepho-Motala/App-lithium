...
public class SmartcashApiService {
    ...
    private static final String REFERENCE_PREFIX = "smartcash";
    private static final String INSTRUMENT_TYPE = "WALLET";
    ...
    public SmartcashPaymentResponse getTransaction(DoProcessorRequest request, DoProcessorResponse response, String token) throws SmartcashException {
        try {
            ...
            response.addRawRequestLog("Smartcash transaction status is requested");
    ...
    public void handlePaymentWebhook(String webhookData) throws Exception {
        ...
        DoProcessorResponse processorResponse = DoProcessorResponse.builder()
            ...
            .build();
        processorResponse.addRawRequestLog("Received Smartcash webhook: " + jsonObjectToPrettyString(webhookData));
    ...
    public String getCustomerWallet(DoProcessorRequest request, DoProcessorResponse response, String token, String msisdn) throws SmartcashException {
        ...
        try {
            String walletId = Arrays.stream(customerSearchResponse.getData().getInstruments())
                .filter(in -> INSTRUMENT_TYPE.equalsIgnoreCase(in.getType()))
                .findFirst()
                .map(SmartcashInstrument::getWalletId)
                .orElseThrow(()-> new Exception("No instrument with type 'WALLET'"));
            response.setProcessorAccount(ProcessorAccount.builder()
                .reference(REFERENCE_PREFIX + "_" + msisdn)
                .providerData(walletId)
            ...
            log.error("Failed to find Smartcash customer by mobile number: " + request.getUser().getTelephoneNumber() + " TransactionId: " + request.getTransactionId(), e);
