...
import lithium.service.cashier.processor.paystack.exeptions.PaystackDeclineTransactionException;
...
public class WithdrawService extends BasePaystackService {
    ...
    public final static List<String> DECLINE_ERRORS_REASON_LIST = List.of(
    ...
    public DoProcessorResponseStatus withdraw(DoProcessorRequest request, DoProcessorResponse response, RestTemplate rest) throws Exception {
        ...
        PaystackTransferResponse paystackTransferResponse;
        try {
            paystackTransferResponse = sendTransfer(request, response, rest,  transferRecipientResponseData.getData().getRecipientCode(), headers);
        } catch (PaystackDeclineTransactionException ex) {
            return buildDeclineResponse(ex.getMessage(), request.getTransactionId(), response);
        }
        if (!checkPaystackTransferResponse(paystackTransferResponse)) {
            return buildDeclineResponse(paystackTransferResponse.getMessage(), request.getTransactionId(), response) ;
        }
    ...
    private PaystackTransferResponse sendTransfer(DoProcessorRequest request, DoProcessorResponse response, RestTemplate rest, String recipientCode, MultiValueMap<String, String> headers) throws PaystackWrongConfigurationException, PaystackDeclineTransactionException, IOException, PaystackServiceHttpErrorException {
        ...
        if (!exchange.getStatusCode().is2xxSuccessful()) {
            ...
            if (DECLINE_ERRORS_REASON_LIST.stream().anyMatch(s -> message.contains(s))) {
                PaystackResponse paystackResponse = mapper.readValue(exchange.getBody(),  PaystackResponse.class);
                throw new PaystackDeclineTransactionException(paystackResponse.getMessage());
            }
