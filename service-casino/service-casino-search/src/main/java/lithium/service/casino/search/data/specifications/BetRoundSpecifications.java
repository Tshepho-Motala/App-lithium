...
public class BetRoundSpecifications {
  ...
  public static Specification<BetRound> domain(final String domainName) {
    return (root, query, cb) -> {
      Join<BetRound, Provider> userJoin = root.join(BetRound_.provider, JoinType.INNER);
      Join<Provider, Domain> domainJoin = userJoin.join(Provider_.domain, JoinType.INNER);
      Predicate p = domainJoin.get(Domain_.name).in(domainName);
      return p;
    };
  }
}
