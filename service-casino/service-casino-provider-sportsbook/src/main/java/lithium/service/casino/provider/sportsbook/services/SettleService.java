...
import lithium.service.casino.provider.sportsbook.storage.repositories.SettlementEntryRepository;
...
import lithium.service.casino.provider.sportsbook.stream.SettlementBalanceAdjustTriggerStream;
...
public class SettleService {
    ...
    @Autowired
    SettlementBalanceAdjustTriggerStream settlementBalanceAdjustTriggerStream;
    ...
    public void validateAndCreateSettle(
        SettleMultiContext context
            ...
            Status485WeeklyWinLimitReachedException, Status478TimeSlotLimitException, Status438PlayTimeLimitReachedException {
        validateRequest(context.getRequest());
        findBet(context);
        validateSha256(context);
        checkDuplicateSubmission(context);
        SW.start("persistPreAdjust");
        persistPreAdjust(context);
        SW.stop();
    }

    /**
     * LSPLAT-11391 PLAT-12568 See swagger on setting balance to 0 due to async adjustment with the
     * default domain currency being returned on the response to allow for backwards compatibility.
     */
    public void updateResponse(SettleMultiContext context) {
        context.getResponse().setBalance(CurrencyAmount.fromCents(0L));
        context.getResponse().setBalanceCurrencyCode(cachingDomainClientService
            .retrieveDomainFromDomainService(context.getDomain().getName()).getCurrency());
        context.getResponse().setTransactionId(context.getSettlement().getId());
    }

    public void triggerAsyncSettlement(SettleMultiContext context) {
        log.info("settlemulti trigger async settlement SettleMultiContext" + context);
        metrics.counter(context.domainName(), EntryPoint.SETTLE_MULTI_ASYNC,
            context.sportsbookBetType(), SportsbookBetStatus.START);
        settlementBalanceAdjustTriggerStream.trigger(context);
    }
    ...
    private void checkDuplicateSubmission(SettleMultiContext context) throws Status409DuplicateSubmissionException {
        ...
        if (settlement != null) {
            context.setSettlement(mapper.map(settlement, lithium.service.casino.provider.sportsbook.context.objects.Settlement.class));
    ...
    public void findBet(SettleMultiContext context) throws Status444ReferencedEntityNotFound {
        HashMap<String, lithium.service.casino.provider.sportsbook.context.objects.Bet> betMap = new HashMap<>();
        for (SettleRequest settleItem : context.getRequest().getSettleRequests()) {
            ...
            lithium.service.casino.provider.sportsbook.context.objects.Bet betObject = mapper.map(bet,
                lithium.service.casino.provider.sportsbook.context.objects.Bet.class);
            betMap.put(bet.getBetId(), betObject);
            context.setUser(mapper.map(bet.getUser(), User.class));
            context.setDomain(context.getUser().getDomain());
            context.setCurrency(betObject.getReservation().getCurrency());

            //LSPLAT-11389 PLAT-12566 User lock has been removed: See git history on this line
        }
        context.setBetMap(betMap);
    }

    private void persistPreAdjust(SettleMultiContext context) {
        ...
        settlement.setCurrency(currencyRepository.findByCode(context.getCurrency().getCode()));
        // Saves the settlement pre-adjustment
        ...
        for (SettleRequest request : context.getRequest().getSettleRequests()) {
            ...
            settlementEntry.setBet(betRepository.findByBetId(request.getBetId()));
            settlementEntry.setType(resolveSportsType(request.getAmount()));
            settlementEntry.setSettlement(savedSettlement);
            settlementEntryList.add(settlementEntryRepository.save(settlementEntry));
        }
        settlement.setSettlementEntries(settlementEntryList);
        // Async update on balanceAfter when balance adjustment is done
        settlement.setBalanceAfter(null);

        log.info("settlemulti created | createdDate=" + savedSettlement.getCreatedDate());
        context.setSettlement(mapper.map(savedSettlement, lithium.service.casino.provider.sportsbook.context.objects.Settlement.class));
    }
        ...
        if (CurrencyAmount.fromAmount(context.getSettlement().getBalanceAfter()).isNegative()) {
            ...
            try {
                ...
                if (cachingDomainClientService.cancelPendingWithdrawalsOnBetResettlement(context.getDomain().getName()) && isMultiResettlement(context)) {
                    ...
                    log.debug("Balance after pending transaction cancellation: " + balance + ", player: " + context.getUser().getGuid());
                    context.getSettlement().setBalanceAfter(CurrencyAmount.fromCents(balance).toAmount().doubleValue());
                    if (balance >= 0) {
                        log.debug("Skip Negative Balance Adjustment for player: " + context.getUser().getGuid());
                        return;
                    }
                }
    ...
    private void callServiceCasino(
        ...
        try {
            ...
            List<lithium.service.casino.provider.sportsbook.context.objects.SettlementEntry> settlementEntries = context.getSettlement().getSettlementEntries();
            for (int i = 0; i < settlementEntries.size(); i++) {
                lithium.service.casino.provider.sportsbook.context.objects.SettlementEntry settlementEntry = settlementEntries.get(i);
                BalanceAdjustmentComponent adjustmentComponent =
                        new BalanceAdjustmentComponent(
                                resolveCasinoSportsType(mapper.map(settlementEntry, SettlementEntry.class)),
            ...
            Settlement settlement = persistPostAdjust(context, response);

            Date now = new Date();
            long diffInMilliseconds = now.getTime() - settlement.getCreatedDate();
            log.info("settlemulti completed | modifiedDate=" + settlement.getModifiedDate() +
                " | latency=" + diffInMilliseconds);
        } finally {
            SW.stop();
        }
    }
    ...
    @Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRES_NEW)
    public Settlement persistPostAdjust(SettleMultiContext context,
        BalanceAdjustmentResponse response) {
        Settlement settlement = settlementRepository.findById(context.getSettlement().getId()).get();

        settlement.setBalanceAfter(CurrencyAmount.fromCents(response.getBalanceCents()).toAmount().doubleValue());
        settlement = settlementRepository.save(settlement);

        // Keeping context in sync;
        context.getSettlement().setBalanceAfter(settlement.getBalanceAfter());

        for (int i = 0; i < response.getAdjustmentResponseComponentList().size(); i++) {
            lithium.service.casino.provider.sportsbook.context.objects.SettlementEntry se
                = context.getSettlement().getSettlementEntries().get(i);
            Optional<SettlementEntry> optionalEntry = settlementEntryRepository
                .findById(se.getId());
            SettlementEntry settlementEntry = optionalEntry.orElse(null);
            if (settlementEntry != null && settlementEntry.getAccountingTransactionId() == null) {
                settlementEntry.setAccountingTransactionId(
                    Long.parseLong(response.getAdjustmentResponseComponentList().get(i)
                        .getExtSystemTransactionId()));
                settlementEntry.setRequestIndex(i);
                settlementEntryRepository.save(settlementEntry);

                // Keeping context in sync;
                se.setAccountingTransactionId(settlementEntry.getAccountingTransactionId());
                se.setRequestIndex(i);
            }
        }
        return settlement;
    }
    ...
    private Long cancelPendingWithdrawals(SettleMultiContext context) throws Status500UnhandledCasinoClientException {
        try {
            log.debug("Cancel pending withdrawals is requested for domain: " + context.getDomain().getName() + " guid: " + context.getUser().getGuid() + " Settlement request: " + context.getRequest());
    ...
    private boolean isMultiResettlement(SettleMultiContext context) {
        Settlement settlement = settlementRepository.findById(context.getSettlement().getId()).get();
        return settlement.getSettlementEntries().stream().anyMatch(e -> {
    ...
    @TimeThisMethod
    public void processSettleMultiAdjustments(SettleMultiContext context)
        throws
        Status401UnAuthorisedException, Status405UserDisabledException,
        Status409DuplicateSubmissionException, Status422DataValidationError,
        Status438PlayTimeLimitReachedException, Status471InsufficientFundsException,
        Status473DomainBettingDisabledException, Status474DomainProviderDisabledException,
        Status478TimeSlotLimitException, Status484WeeklyLossLimitReachedException,
        Status485WeeklyWinLimitReachedException, Status490SoftSelfExclusionException,
        Status491PermanentSelfExclusionException, Status492DailyLossLimitReachedException,
        Status493MonthlyLossLimitReachedException, Status494DailyWinLimitReachedException,
        Status495MonthlyWinLimitReachedException, Status496PlayerCoolingOffException,
        Status511UpstreamServiceUnavailableException, Status550ServiceDomainClientException {

        SW.start("callServiceCasino");
        preAdjustQueueValidation(context);
        ...
        SW.stop();
        if (cachingDomainClientService.allowNegativeBalanceAdjustment(
    ...
    private void preAdjustQueueValidation(SettleMultiContext context)
        throws Status409DuplicateSubmissionException,
        Status422DataValidationError {

        Settlement settlement = settlementRepository.findByRequestId(
            context.getSettlement().getRequestId());
        if (settlement == null) {
            log.debug("A settlement has been received on the sportsbook-settlement-queue which "
                + "does not exist in the DB | settleMultiContext {}", context);
            throw new Status422DataValidationError(
                "A settlement has been received on the sportsbook-settlement-queue"
                    + " which does not exist in the DB");
        }

        List<SettlementEntry> settlementEntries = settlement.getSettlementEntries();
        // All settlements that has not been adjusted will have a balanceAfter
        // All settlements that has not been adjusted will have an accountingTransactionId
        if (settlement.getBalanceAfter() == null || (settlementEntries != null
            && !settlementEntries.isEmpty()
            && settlementEntries.stream().anyMatch(settlementEntry ->
            settlementEntry.getAccountingTransactionId() == null))) {
            return;
        }

        throw new Status409DuplicateSubmissionException(
            "A Settlement has been found on the queue which has already been processed and will"
                + " therefore regard the message for settlementId = " + settlement.getId()
                + " as processed.");
    }
}
