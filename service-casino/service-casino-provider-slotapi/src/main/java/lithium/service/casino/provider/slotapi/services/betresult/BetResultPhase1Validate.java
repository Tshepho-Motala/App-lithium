...
import lithium.service.casino.provider.slotapi.api.exceptions.Status404OriginalTransactionNotFound;
    ...
    private void validateKind(BetResultContext context, BetResultRequest request) throws Status422DataValidationError {
        switch (request.getKind()) {
            ...
            case VOID:
                ...
                Bet bet = betRepository.findByBetTransactionId(request.getBetResultTransactionId());
                if(bet == null) {
                    throw new Status404OriginalTransactionNotFound();
                }
                if (bet.getAmount() < request.getReturns()) {
                    throw new Status422DataValidationError("Returns may not be more than the bet amount on a VOID");
                }
                break;
            default:
