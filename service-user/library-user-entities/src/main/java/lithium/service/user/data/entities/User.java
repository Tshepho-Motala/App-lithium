...
@ToString(exclude = {"current", "userEvents", "passwordPlaintext", "passwordHash", "groups","userCategories"})
...
//	)
  ...
  @Pattern(regexp = "^$|[_A-Za-z0-9-\\+]+(\\.[_+A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$", message = "Invalid email address")
  @Column(name = "EMAIL")
  private String email;
  private String deletedEmail;
  @Pattern(regexp = "^[\\p{L}\\p{Pd}\\p{Po}\\p{Sk}+ ]*$", message = "Invalid characters in First Name")
  @Column(nullable = false)
  @Size(max = 35, message = "No more than 35 characters")
  /**
   * If updating this, remember to update SignupService.
   * lithium.service.user.services.SignupService
   */
  private String firstName;
  @Pattern(regexp = "^[\\p{L}\\p{Pd}\\p{Po}\\p{Sk}+ ]*$", message = "Invalid characters in Last Name")
  @Column(nullable = false)
...
//		log.info(""+UUID.randomUUID().hashCode());
  ...
  @JsonIgnore
  public List<Long> getPlayerTagIds() {
  ...
  public boolean hasGroups(){
    return Optional.ofNullable(this.getGroups())
