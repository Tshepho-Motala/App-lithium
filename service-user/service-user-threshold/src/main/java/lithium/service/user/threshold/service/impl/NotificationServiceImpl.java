...
public class NotificationServiceImpl implements NotificationService {
  ...
  private CurrencyAmount getAmount(ProcessingContext context) {
    switch (context.getThreshold().getGranularity()) {
      ...
      case GRANULARITY_YEAR -> {
        return CurrencyAmount.fromAmount(context.getPlayerThresholdHistory().getAnnualLossLimitUsed());
      }
      default -> {
  ...
  public void sendLossLimitVisibilityToPlayerInbox(ProcessingContext context) {
    if (!thresholdProperties.sendNotifications()) {
      log.debug("Threshold notifications switched off globally. Not sending LossLimitVisibilityNotification. ({})", context);
      return;
    }
