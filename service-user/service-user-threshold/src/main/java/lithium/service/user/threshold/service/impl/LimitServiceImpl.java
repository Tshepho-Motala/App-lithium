...
public class LimitServiceImpl implements LimitService {
  ...
  throws Status500InternalServerErrorException
  {
    PlayerLimitV2Dto playerLimit = limitInternalSystemService.findPlayerLimitV2WithNetLoss(user.getDomain().getName(), user.getGuid(),
        userService.playerAge(user), threshold.getGranularity().granularity(), EType.TYPE_LOSS_LIMIT.type());
  ...
  throws Status500InternalServerErrorException
  {
    ...
    if (ObjectUtils.isEmpty(limit) || ObjectUtils.isEmpty(limit.getLimitAmount())) {
      log.debug("No configured loss limit found.");
      return;
    }
  ...
  throws Status500InternalServerErrorException
  {
    ...
    BigDecimal depositAmountTotal = CurrencyAmount.fromCents(sattd.getSummaryAccountTransactionType().getCreditCents()).toAmount();

    boolean playerReachedDepositThreshold = (depositAmountTotal.compareTo(context.getThreshold().getCurrent().getAmount()) >= 0);
