...
public class UserAttributesDataMigrationJob {
  ...
  public void migrateUserAttributesData(boolean dryRun, int pageSize, Long delay) throws InterruptedException {
    ...
    if (!JobState.isStarted) {
      ...
      do {
        ...
        total += migratedAccountsCount;

        log.info(":: Migration of users attributes : migrated " + total + " users");
  ...
  public void migrateUserAttributesData(boolean dryRun, List<String> guids) {
    log.info("/service-user/user-attributes, isMigrationJobStarted={}", JobState.isStarted);
    if (!JobState.isStarted) {
      JobState.start(dryRun);

      long migratedAccountsCount = migrate(guids) ;

      log.info(":: Migration of users attributes : migrated " + migratedAccountsCount + " users from " + guids.size());
  ...
  private long migrate(List<String> guids) {
    List<UserAttributesData> attributesData = migrationService.getUserAttributesForSyncByGuids(guids);
    return sendDataToStream(attributesData);
  }
  ...
  private long sendDataToStream(List<UserAttributesData> attributesData) {
